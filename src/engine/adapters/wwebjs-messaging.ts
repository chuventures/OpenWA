...
const logger = createLogger('WwebjsMessaging');
 ...
 * not be read back (`Msg.get` miss, `Injected/Utils.js:585`). Nothing here can tell those apart.
 * Upstream reports it as a failed send, but that made OTP/alert callers retry and duplicate messages
 * that had already landed (fork fix 845b00a), so this fork reports it with the empty-id success
 * sentinel instead; delivery certainty belongs to `message.ack` / chat history, not the sync HTTP
 * response. wwebjs's own typings hide the case entirely — `index.d.ts` declares `Promise<Message>`,
 * so `strict` never flagged these reads.
 *
